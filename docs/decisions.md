...
**Why:** the plugin surface was homegrown twice over — a `pkgutil` walk for built-ins, a bare entry-point scan for externals, and a `Protocol` whose only enforcement was four attribute names. pluginkit is in-house, strictly typed, and has no runtime dependencies, so the seam a future pack repository registers through is the same one the built-ins use, and a new host capability is a new extension point rather than another scan written beside the old one. A `Contribution` naming modules rather than carrying callables keeps the import lazy — `d2w --help` never imports FastMCP — and makes what a plugin provides a value the host can inspect, sort, and report on.

**Alternatives rejected:** keeping the `Protocol` and adding a second group for packs (two mechanisms for one job, and no version in either group name); a registry module listing every plugin (the thing the `pkgutil` walk existed to avoid); pydantic plugin objects for the symmetry with the rest of the codebase (pydantic's class-only `__signature__` descriptor raises during pluginkit's attribute scan — the `Contribution` is the model, the plugin is not).
