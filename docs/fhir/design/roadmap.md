...
| `Makefile` | `help / setup / upgrade / generate / validate / cache-init / sushi / build / clean / clean-all / refresh`, `D2W ?= uv run d2w`, `TX_SERVER ?= http://tx.fhir.org`, `JAVA_HEAP` derived once per build from the memory docker reports less 2G and capped at 31g (the publisher JVM's ceiling - above what the machine holds, the kernel's out-of-memory killer takes the publisher and `build` reports exit 137 with the ceiling, the memory figure and the peak reached), and the `fhir-ig-cache` named volume. |
