...
| `Makefile` | `help / setup / upgrade / generate / validate / cache-init / sushi / build / clean / clean-all / refresh`, `D2W ?= uv run d2w`, `TX_SERVER ?= http://tx.fhir.org`, `JAVA_HEAP` derived from the docker VM less 2G (the publisher JVM's ceiling - above what the VM holds, the kernel OOM-kills the build and `build` reports exit 137 with the ceiling, the VM and the peak reached), and the `fhir-ig-cache` named volume. |
