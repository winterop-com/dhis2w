...
| **Serve** - conformance | `build_server_capability` (`capability.py:160`) and `build_served_metadata` (`metadata.py:32`) - both re-exported | Nothing beyond the route | Unchanged | `LIBRARY` |
...
6. `RegisterSurface.resolve(TrackedEntityIndex.from_store(project, store), settings.tracked_entities)`
   (`:136`) and `build_served_metadata(...)` (`:145`), both into a `ServeContext` on
