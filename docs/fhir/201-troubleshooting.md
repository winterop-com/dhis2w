...
| `Invalid value: --registry-path <path> names no directory: the project reads it from its own root, which puts it at <resolved>.` | `--registry-path` pointed at a directory that does not exist. The path is relative to the new project's own root, not to the shell's working directory. | Scaffold the registry project at that path first, or drop the flag and let `make build` install the package from its id and version. |
| `` error: `<name>` is an example, not a template. `` followed by what it demonstrates | `--template` named a guide of `examples/fhir/igs/` whose `template.toml` says `scaffolds = false` - an exhibit rather than a template. | Read the example where it lives; `--list-templates` names every guide you can scaffold from. |
...
| `Exception in thread "main" java.lang.OutOfMemoryError: Java heap space` mid-validation, a Java stack trace, partial phases logged | The ceiling is too small for the guide. This is a real build error, not a kill. | Raise it - `make build JAVA_HEAP=<larger>` - and give docker more memory with it, since the derivation already hands a build what docker reports less 2 GB and going above that trades this failure for an exit-137 kill in the write phases. On a machine that small the two failures meet: on one 16 GB docker VM, `8g` completes a national guide where `4g` dies here and `10g` is OOM-killed at Jekyll, so a derived `14g` is killed there and `8g` set by hand is the way through. |
| The publisher exits `137` and the build reports `BUILD KILLED - out of memory` | `128 + 9` - SIGKILL, and the container's cgroup counted an `oom_kill`, so this is the kernel's out-of-memory killer during the peak-memory phases. The ceiling is too large for the machine. | The build prints this one in full: the ceiling, the memory docker reports, the peak reached, and the containers running when it was sampled. Stop those and run again - a publisher build wants the machine to itself - or give docker more memory (Docker Desktop: Settings > Resources > Memory; on Linux docker uses the machine's own memory), or drop the ceiling with `make build JAVA_HEAP=<smaller>`. `ig/output` holds whatever the last completed build wrote, or is partial - the site is written last, so nothing from this run is trustworthy. |
| The publisher exits `137` and the build reports `BUILD KILLED` without the out-of-memory line | A SIGKILL the kernel's out-of-memory killer did not count: a `docker stop` on a container ignoring SIGTERM, a `docker kill`, a timeout around the build. A host on cgroup v1 counts none either way. | Look outside the build: a CI step timeout, a `docker stop` or `docker kill` from another terminal, a machine that slept. `ig/output` is in the same state as above - the last completed build's, or partial. |
...
| Symptom (literal text) | Cause | Fix |
| --- | --- | --- |
| `error: no compiled IG at ig/fsh-generated/resources - run` `` `d2w fhir generate` ``, `then` `` `make sushi` `` `in the project, and serve again.` | The default mode serves what SUSHI wrote, and this project has never been compiled. In a guide depending on a registry package, the message also names the registry build the compile waits on. | What the message says - or `d2w fhir serve --live` to serve straight from the instance, no compile needed. |
...
| `` error: the capture UI bundle at .../dhis2w_fhir_serve/static is older than the frontend source - it was built from <fingerprint> and the source now reads <fingerprint>. Run `make ui` to rebuild it. `` | A checkout whose frontend source moved since the bundle was built - a pull, a branch, or an edit of your own. The browser would otherwise run the older JavaScript with nothing saying so. | `make ui`. Only a checkout is graded this way; an installed wheel carries no frontend source to compare against. |
...
| `error: N response(s) rejected by DHIS2; exiting 1 - read the import summary, fix the instance or the data, and forward again` | DHIS2's import refused the payloads; each receipt moved to `rejected/` beside its import report. Common codes: `E1029` (unit outside the form's assignment), `E8023` (missing attribute option combo), `E8025` (a combo whose category options are not available at that organisation unit), `E1064` (duplicate unique attribute value). | Read `reports/fhir-forward-report.md` and the per-receipt reports; fix, then forward again. |
