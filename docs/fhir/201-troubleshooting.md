...
| `Exception in thread "main" java.lang.OutOfMemoryError: Java heap space` mid-validation, a Java stack trace, partial phases logged | The guide does not fit the publisher's JVM heap, `8g` by default. | Raise `JAVA_HEAP` above `8g` together with the docker VM, which needs heap + ~2 GB or the fix trades this failure for an exit-137 kill in the write phases: `make build JAVA_HEAP=10g` on a 12 GB VM. On one 16 GB VM, `8g` completes a national guide where `4g` dies here and `10g` is OOM-killed at Jekyll. |
| The publisher exits `137` with `ig/output` empty afterwards | `128 + 9` - SIGKILL from the kernel's OOM killer during the peak-memory phases. | Give the docker VM more memory (heap + ~2G), or drop the publisher's `-Xmx` below the `8g` default (`make build JAVA_HEAP=4g`) to fit the box. Confirm with `docker inspect <container> --format '{{.State.OOMKilled}}'`. |
