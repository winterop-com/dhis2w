...
| `Exception in thread "main" java.lang.OutOfMemoryError: Java heap space` mid-validation, a Java stack trace, partial phases logged | The ceiling is too small for the guide. This is a real build error, not a kill. | Raise it - `make build JAVA_HEAP=<larger>` - and raise the docker VM with it, since the derivation already gives a build the VM less 2 GB and going above that trades this failure for an exit-137 kill in the write phases. On one 16 GB VM, `8g` completes a national guide where `4g` dies here and `10g` is OOM-killed at Jekyll. |
| The publisher exits `137` with `ig/output` empty afterwards | `128 + 9` - SIGKILL from the kernel's OOM killer during the peak-memory phases. The ceiling is too large for the box. | The build prints this one in full: the ceiling, the VM, the peak reached, and any other containers holding the VM. Stop those and run again - a publisher build wants the machine to itself - or raise the VM, or drop the ceiling with `make build JAVA_HEAP=<smaller>`. Confirm with `docker inspect <container> --format '{{.State.OOMKilled}}'`. |
