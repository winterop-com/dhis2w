...
| [What goes in](301-what-goes-in.md) | the six selection tables, `[generate.tracked_entity_types]`, `[generate.organisation_units]`, `[generate.examples]`, `[ips.identity]`, `[ips.sections]` - which of your DHIS2 metadata the guide covers, which attribute carries which fact about a person, and which recorded values feed which section of a patient summary |
...
| `[generate]` | [`hostile_names`](301-generation.md#hostile_names) | unset (the run asks) | whether a DHIS2 name carrying `<` refuses the run or is published in rewritten wording, and whether a DHIS2 code carrying a space is published hyphenated |
