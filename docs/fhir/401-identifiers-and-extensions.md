...
| `assigns` | `id` | 0..* | A question this rule computes the answer to; one entry per question |
...
**`assigns` is the one sub-extension a client has to act on.** A DHIS2 `ASSIGN`
rule computes an answer on import and refuses a payload whose answer is neither
empty nor byte-equal to the value it calculated, with:

```
E1307 Generated by ProgramRule (`tZysl1WgPHF`) - Unable to assign value to
DataElement `GieVkTxp4HH`. The provided value must be empty or match the
calculated value `-Infinity`
```

A calculated value is often one no answer expresses at all - `-Infinity` comes
out of `d2:log(0)` - so **the only answer DHIS2 always accepts for a question an
`ASSIGN` rule names is no answer.** Send the question empty. The rule's own
`condition` is the DHIS2 expression rather than anything a client can evaluate,
and even a client that could evaluate it would have to match DHIS2's arithmetic
exactly; leaving the question unanswered is both simpler and always right.

Every example this toolchain publishes does exactly that: the examples target and
`d2w fhir generate load-set` draw no value for an assigned question, and the run
raises a note per form naming the rules and the questions they compute.
`d2w fhir check-artifacts` files a warning-level finding for any published example
that answers one, generated or hand-authored.

Each entry is one question's UID, which is that question's `linkId` on the same
form. A rule computing several questions carries several entries; a rule of a
program whose question another stage asks carries an entry no item of this form
matches, which is the same fact every tier-3 rule states about itself.

```
* extension[D2ProgramRule][+].extension[rule].valueId = "wfw6yKp1s2r"
* extension[D2ProgramRule][=].extension[name].valueString = "Derive the log of the reading"
* extension[D2ProgramRule][=].extension[condition].valueString = "true"
* extension[D2ProgramRule][=].extension[action].valueCode = #ASSIGN
* extension[D2ProgramRule][=].extension[assigns][+].valueId = "ySU9WWHNoVG"
```
