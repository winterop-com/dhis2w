...
- **Tracker + event programs** — Child Programme (WITH_REGISTRATION, two non-repeatable immunization stages), ANC follow-up (WITH_REGISTRATION, one **repeatable** "ANC visit" stage with a compulsory visit-number element - the repeated-visit pattern most real tracker deployments use; two enrolled women with four and one visit events on fixed UIDs, see `infra/scripts/seed/anc_program.py`), Antenatal care visit (WITHOUT_REGISTRATION), Supervision visit (WITHOUT_REGISTRATION). 500 tracked entities, 12 sample supervision events covering 2024 monthly. Every seeded person carries a Unique ID (`lZGmxYbs97q`) minted by `person_identifier` in `infra/scripts/seed/loader.py` - deterministic in the fixture row, so a register shows an identifier and an identifier search finds someone.
...
- **FHIR fixtures** for the paths the play bundle leaves untested. Three `Attribute` definitions (`AtrFhirOpS1` coded, `AtrFhirDsQ1` uncoded, `AtrFhirOrU1` coded and unique) with a value each on an option set, a data set, and the org-unit root, so the `D2AttributeValue` extension is exercised with `attributeCode` both present and absent. Plus a text-handling set: option set `OsFhirEscS1` and category `CatFhirEsc1`, both named with `<`, `>`, and `&`, both carrying Lao `NAME` translations, with three options and three category options between them covering a coded value, an uncoded one, and a code holding a space. Those are what give the IG generated from this stack its escaped page furniture, its translation elements, and its uncoded-code fallbacks - all of which measured zero before they existed. See `infra/scripts/seed/fhir_variations.py`, whose docstring records the two variations that turned out to be unseedable and why.
