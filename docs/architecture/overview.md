...
v43 is the canonical baseline: new behaviour is written against the v43 tree first and copied to v41 and v42, and the trees diverge per-file as version-specific quirks land (CategoryCombo COC regeneration on v43, the `categorys` -> `categories` rename, v41's missing `OAuth2ClientCredentialsAuthScheme`, etc.). The version-neutral packages (`dhis2w-fhir`, `dhis2w-fhir-serve`, `dhis2w_core.security_core`) import their generated models from `dhis2w_client.generated.v42.*`; moving them to the v43 tree is on the roadmap.
