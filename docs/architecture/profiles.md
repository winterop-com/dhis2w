...
When the field is omitted, the CLI consults the `DHIS2_VERSION` env var (the vXX key: `v41` / `v42` / `v43`); when that's also unset, it defaults to the canonical v43 tree. A `version` pin wins over `DHIS2_VERSION`. `make verify-examples` follows the same `resolve_startup_version()` chain: the version-neutral `examples/{cli,client,mcp}/` set runs whatever the major, and a v41-pinned profile adds `examples/client/v41/` on top of it. On an unpinned profile, `make verify-examples DHIS2_VERSION=v43` swaps in the v43 variants without editing the profile.
