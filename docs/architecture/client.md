...
The client library is deliberately small. The big ideas — auth, version dispatch, typed dispatch — are in separate modules; the client itself is a thin async httpx2 wrapper that glues them together.
...
Dhis2Client(
    ...
    http_limits=httpx2.Limits(...),  # optional; connection-pool sizing
)
```

## Connection pool tuning

`Dhis2Client` runs one `httpx2.AsyncClient` shared across every concurrent call on that instance. The pool is where concurrency lives — `asyncio.gather` spawns N awaitables, but only the pool-sized subset is actually in flight against DHIS2 at any moment.

**Defaults** (httpx2's own): **100 max connections**, **20 max keepalive connections**, **5s keepalive expiry**. Sensible for most scripts; rarely the right answer for batch workflows.
...
Override via `httpx2.Limits` at construction time:
...
tight = httpx2.Limits(max_connections=10, max_keepalive_connections=5)
...
tight = httpx2.Limits(max_connections=15, max_keepalive_connections=5)
...
1. Builds the underlying `httpx2.AsyncClient` (connection pool).
...
- **`httpx2.AsyncClient` is held as an attribute, not constructed per call.** Connection pooling matters for MCP tools that an agent hits dozens of times per session.
...
- **No retries, no backoff, no circuit breakers — yet.** Those belong in a transport layer we'd wire via `httpx2.AsyncHTTPTransport` when we know what the failure modes actually are in practice. Premature policy would encode the wrong defaults.
