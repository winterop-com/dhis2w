...
| **security** | The read-only security posture scanner — version and patch posture, transport and security headers, password policy and registration settings, authority risk categorisation, role and account audits, installed-apps inventory, anonymous-access and default-credential probes, public-metadata sharing, route targets, personal access tokens, external login methods, auditing posture, and a resumable `audit` runner writing Markdown / plaintext / CSV / HTML reports — is the [`dhis2w-security`](https://github.com/winterop-com/dhis2w-security) pack, installed alongside the CLI with `uv tool install dhis2w-cli --with dhis2w-security` (or `uv add dhis2w-security` in a project). |
