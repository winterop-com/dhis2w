...
* `--name <str>`: SUSHI name (default: derived from --id). FHIR computer-friendly: an upper-case letter, then up to 254 letters, digits or underscores. SUSHI rewrites anything else without saying so, so a name outside that shape is refused here.
* `--title <str>`: IG title (default: derived from --name). A title carrying &#x27;&lt;&#x27; or &#x27;&gt;&#x27; is refused: the IG publisher strict-parses the pages it writes the title into, and aborts the build in its last pass.
...
* `--registry-package <path>`: The organisation-unit registry package to check this guide&#x27;s unit references against, when no `path` checkout answers. Only for a guide naming `[generate.organisation_units.registry]`.
* `--help`: Show this message and exit.
...
* `--ui / --no-ui`: Serve the capture UI at `/` alongside the FHIR routes, overriding `[serve] ui`. The bundle is mounted around them and shadows none of them, and the run names the build it is serving. In a checkout, a bundle older than the frontend source beside it is refused rather than served: `make ui` builds it, and the refusal says so.
