...
- **Streaming data-value-set import** — `client.data_values.stream(source, content_type=...)` feeds httpx2's chunked transfer directly from a `Path`, `bytes`, sync / async iterable, or async generator. JSON / XML / CSV / ADX.
- **Streaming analytics export** — `client.analytics.stream_to(destination, *, params, endpoint="/api/analytics.json")` pipes httpx2's chunked response straight to disk via `aiter_bytes`.
...
The workspace carries every domain for every install and about 18,000 tests across eleven members, while usage concentrates in `dhis2w-client` and `dhis2w-core`. The next big piece of work splits it the way `dirigent` is built: a small core with extension points on [`pluginkit`](https://pypi.org/project/pluginkit/) (strictly typed, generics-first, `ExtensionPoint` / `Extension` / `PluginManager`, entry-point discovery, no runtime dependencies), with FHIR, security, MCP and similar domains as plugins.

- `dhis2w-client` and `dhis2w-core` stay central; every other domain becomes a plugin that installs on its own.
- [`dhis2w-integration`](https://github.com/winterop-com/dhis2w-integration), like `dirigent-integration`, holds the LLM benchmark harness and the tests that run across several plugins at once, so cross-plugin behaviour has one home.
- The seam exists: `dhis2w_core.plugin` is a pluginkit host, every plugin answers the `contribute` extension point with a `Contribution`, and a pack registers through the `dhis2w.plugins.v1` entry-point group. `dhis2w-security` is the first pack in its own repository, and `dhis2w-integration` assembles the host and every pack into one environment.
...
- **Other measured perf items** (independent; from a sweep — file:line + rough effort/payoff):
  ...
  - **HTTP pool defaults untuned** (`dhis2w-client/v42/client.py`) — httpx2 defaults (100 max / 20 keepalive) cap high-fan-out `asyncio.gather` callers; document/raise `http_limits` for bulk workflows.
...
- **Multi-backend `ModelBackend` (Ollama / llama.cpp)** — the local-model validation harness (`dhis2w_integration.bench.backend`, in [`dhis2w-integration`](https://github.com/winterop-com/dhis2w-integration)) abstracts model lifecycle (list / load / unload / server / chat-url) behind a `ModelBackend` Protocol, with `LmStudioBackend` the only implementation today (selectable via `MODEL_BACKEND`). Adding `OllamaBackend` (auto-loads on first request, evicts by keep-alive, size via `/api/tags`) and `LlamaCppBackend` (one model per `llama-server` process, or llama-swap) would let the harness drive any local runtime. The inference call is already portable (OpenAI-compatible `/v1`); only lifecycle differs. This is really a standalone-tool concern — the general lane, `ModelBackend` and the backend listing are DHIS2-agnostic and could extract out of the integration entirely (only the bridge lane is inherently dhis2w). Interesting side project; parked until a non-LM-Studio runtime is actually in play.
...
Each plugin gets one PR: respx-driven happy-path + error-shape tests at the service layer, `typer.testing.CliRunner` smoke tests for every CLI verb, in-process `httpx2.AsyncClient` integration through the FastMCP server for the MCP wrappers. Estimate: 5–7 PRs total, ~3–4 days of focused work. Worth doing before pinning per-package gates (B2).
...
- **Bulk RFC 6902 patch** — `client.metadata.patch_bulk(resource, [(uid, ops), ...], concurrency=8)` + `patch_bulk_multi(...)` (#187). Client-side fan-out through a worker pool of `min(concurrency, len(work))` coroutines drawing from one shared cursor; per-UID failures land in `BulkPatchResult.failures` (with `uid` / `resource` / `status_code` / `message`) instead of raising. Building block for future CLI-level bulk verbs.
- **Bulk sharing** — `client.metadata.apply_sharing_bulk(resource_type, uids, sharing)` + `apply_sharing_bulk_multi(by_resource, sharing)` fan out one `SharingBuilder` payload across many UIDs through the same bounded worker pool, `concurrency` coroutines at most. CLI surface as `d2w metadata share <type> [UID...]` with `--public-access` / `--user-access UID:access` / `--user-group-access UID:access` (repeatable) + stdin UID input via `-` so `metadata list ... \| jq -r .id \| xargs metadata share` composes. Per-UID failures land in `BulkSharingResult.failures` with the same row-level table renderer used by `rename` / `retag`.
...
- A full FHIR resource model set — in both senses: none is hand-built, and
  `fhir.resources` is not adopted as a dependency. The design it would displace is
  deliberate: the FHIR shapes we publish are generic (a plain `QuestionnaireResponse`,
  standard `CodeSystem`/`ValueSet`, `Location`, a standard document `Bundle`) with every
  DHIS2 specificity carried in the D2 extensions — a portable pattern another system
  could adopt with its own extension vocabulary — and a model or element lands only in
  the same PR as the feature that needs it. The evaluation engine walks FHIR data as
  data, which is neutrality, not coverage.
