...
def test_empty_inventory_emits_only_inventory_finding() -> None:
    ...
    findings = evaluate_routes([]).findings
...
def test_public_route_produces_only_inventory() -> None:
    ...
    assert _titles(evaluate_routes([_PUBLIC]).findings) == {"Route inventory"}
...
def test_private_ipv4_host_is_high() -> None:
    ...
    finding = _by_title(evaluate_routes([target]).findings)["Route proxies to a private or internal host"]
...
def test_private_ip_ranges_are_high(host: str) -> None:
    ...
    assert "Route proxies to a private or internal host" in _titles(evaluate_routes([target]).findings)
...
def test_metadata_endpoints_are_high_metadata_finding(host: str) -> None:
    ...
    titles = _titles(evaluate_routes([target]).findings)
...
def test_metadata_endpoint_does_not_double_fire_private_address() -> None:
    ...
    by_title = _by_title(evaluate_routes([target]).findings)
...
def test_allows_subpaths_is_medium() -> None:
    ...
    finding = _by_title(evaluate_routes([target]).findings)["Route allows arbitrary subpaths"]
...
def test_carries_auth_is_info_and_shows_identity_not_secret() -> None:
    ...
    finding = _by_title(evaluate_routes([target]).findings)["Route carries upstream credentials"]
...
def test_no_required_authorities_on_active_route_is_medium() -> None:
    ...
    finding = _by_title(evaluate_routes([target]).findings)["Route has no required authorities"]
    assert finding.severity is Severity.MEDIUM
...
def test_no_required_authorities_suppressed_when_disabled() -> None:
    ...
    assert "Route has no required authorities" not in _titles(evaluate_routes([target]).findings)
...
def test_inventory_counts_active_and_disabled() -> None:
    ...
    inventory = _by_title(evaluate_routes(routes).findings)["Route inventory"]
