...
def test_discover_plugins_default_matches_explicit_v43() -> None:
    """`discover_plugins()` with no arg walks the v43 plugin tree."""
    default_names = sorted(p.name for p in discover_plugins())
    explicit_names = sorted(p.name for p in discover_plugins("v43"))
