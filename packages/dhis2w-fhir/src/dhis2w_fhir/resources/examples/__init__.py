    ...
    def to_notes(self) -> list[GenerateNote]:
        ...
        if self.unknown_data_elements:
            notes.append(
                aggregate_generate_note(
                    ...
                    f"{pluralize(len(self.unknown_data_elements), 'captured value')} "
                    f"{verb_for_count(len(self.unknown_data_elements), 'references', 'reference')} a data "
                    "element the questionnaire does not ask for; skipped",
        ...
        if self.untyped_values:
            notes.append(
                aggregate_generate_note(
                    ...
                    f"{pluralize(len(self.untyped_values), 'example answer')} could not be cast to the "
                    "declared FHIR type; answered as text",
                    self.untyped_values,
                )
            )
        if self.uncoded_options:
            notes.append(
                aggregate_generate_note(
                    GenerateNoteCategory.SKIPPED_QUESTION,
                    f"{pluralize(len(self.uncoded_options), 'example answer')} "
                    f"{verb_for_count(len(self.uncoded_options), 'selects', 'select')} an option the "
                    "CodeSystem holds no concept for; left unanswered",
        ...
        if self.uncoded_attribute_option_combos:
            notes.append(
                aggregate_generate_note(
                    ...
                    f"{pluralize(len(self.uncoded_attribute_option_combos), 'example')} "
                    f"{verb_for_count(len(self.uncoded_attribute_option_combos), 'is', 'are')} keyed to an "
                    "attribute option combo the form's published vocabulary holds no concept for; the "
                    "response carries no D2AttributeOptionCombo extension",
        ...
        if self.unpublished_organisation_units:
            notes.append(
                aggregate_generate_note(
                    GenerateNoteCategory.SKIPPED_QUESTION,
                    f"{pluralize(len(self.unpublished_organisation_units), 'example answer')} "
                    f"{verb_for_count(len(self.unpublished_organisation_units), 'references', 'reference')} "
                    "an organisation unit outside the org-unit selection, which the IG publishes no "
                    "Location for; left unanswered",
        ...
        if self.periodless_data_sets:
            notes.append(
                aggregate_generate_note(
                    GenerateNoteCategory.INSTANCE_DATA_GAP,
                    f"{pluralize(len(self.periodless_data_sets), 'data set')} "
                    f"{verb_for_count(len(self.periodless_data_sets), 'has', 'have')} no resolvable "
                    "reporting period; those examples carry no D2Period extension and declare the base "
                    "QuestionnaireResponse instead of the aggregate response profile",
        ...
        if self.unauthored_responses:
            notes.append(
                aggregate_generate_note(
                    GenerateNoteCategory.INSTANCE_DATA_GAP,
                    f"{pluralize(len(self.unauthored_responses), 'example')} "
                    f"{verb_for_count(len(self.unauthored_responses), 'carries', 'carry')} an occurrence "
                    "timestamp FHIR cannot express as a dateTime; authored omitted and the base "
                    "QuestionnaireResponse declared instead of the event or tracker event response profile",
        ...
        if self.incomplete_tracker_responses:
            notes.append(
                aggregate_generate_note(
                    GenerateNoteCategory.INSTANCE_DATA_GAP,
                    f"{pluralize(len(self.incomplete_tracker_responses), 'example')} "
                    f"{verb_for_count(len(self.incomplete_tracker_responses), 'lacks', 'lack')} the "
                    "enrollment or tracked entity a tracker event carries; the base QuestionnaireResponse "
                    "is declared instead of the tracker event response profile",
    ...
    *,
    ...
    if unanswerable:
        build.notes.append(
            aggregate_generate_note(
                GenerateNoteCategory.SKIPPED_QUESTION,
                f"{pluralize(len(unanswerable), 'question')} "
                f"{verb_for_count(len(unanswerable), 'takes', 'take')} an attachment, a geometry document, "
    ...
    if indistinct:
        build.notes.append(
            aggregate_generate_note(
                GenerateNoteCategory.INSTANCE_DATA_GAP,
                f"{pluralize(len(set(indistinct)), 'unique tracked entity attribute')} "
                f"{verb_for_count(len(set(indistinct)), 'has', 'have')} a value type with no room for a "
                "corpus-distinct value, so the answers repeat; DHIS2 refuses every registration after the "
                "first with E1064",
