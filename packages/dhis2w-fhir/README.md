...
The package registers itself through the `dhis2w.plugins.v1` entry-point group, so installing it next to `dhis2w-cli` adds the `d2w fhir` commands. Naming of generated artifacts is configurable via `[generate.naming]` in `fhir.toml`.
