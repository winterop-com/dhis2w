...
def test_details_prints_every_response_and_writes_the_report_beside_it(forward_project: Path) -> None:
    """`--details` is the firehose: one row per receipt with what became of it and why, report and all.

    The report is written whatever the flags, because a file left on disk from an earlier run is a
    file the next reader takes for this one's verdict.
    """
    result, _ = _invoke(["--no-progress", "--details"], _report(forward_project))
    written = forward_project / "reports" / "fhir-forward-report.md"
    assert written.exists()
    assert "accepted-1" in written.read_text(encoding="utf-8")
    assert "also written to" in result.output
    for response_id in ("accepted-1", "rejected-1", "refused-1"):
        assert response_id in result.output
    assert "do not match" in result.output


def test_the_report_is_refreshed_by_a_details_run_rather_than_left_as_it_was(forward_project: Path) -> None:
    """The stalest thing a run can leave behind is the previous run's verdict under this run's name."""
    stale = forward_project / "reports" / "fhir-forward-report.md"
    stale.parent.mkdir(parents=True, exist_ok=True)
    stale.write_text("# an earlier run\n\n- Counts: 9 spooled, 9 accepted\n", encoding="utf-8")

    _invoke(["--no-progress", "--details"], _report(forward_project))

    refreshed = stale.read_text(encoding="utf-8")
    assert "an earlier run" not in refreshed
    assert "9 spooled" not in refreshed
...
def test_an_empty_spool_says_what_fills_it(forward_project: Path) -> None:
    ...
    assert "0 spooled" in (forward_project / "reports" / "fhir-forward-report.md").read_text(encoding="utf-8")
...
def _accepted_outcome() -> ForwardOutcome:
    """One response DHIS2 took."""
...
def _rejected_outcome() -> ForwardOutcome:
    """One response DHIS2 refused."""
    return ForwardOutcome(
        response_id="rejected-1",
        questionnaire="http://example.org/fhir/Questionnaire/d2-pr-surveillance",
        target_kind=ConversionTargetKind.EVENT,
        ...
        import_outcome=ForwardImportOutcome(
            ...
            issues=(
                ForwardImportIssue(
                    error_code="E1307",
                    subject="Ev1aaaaaaaa",
                    message="Generated by ProgramRule (`Rule1aaaaaa`) - Unable to assign value to DataElement.",
                ),
            ),
        ),
        spool_path=".serve/responses/received/rejected-1.json",
...
def _refused_outcome() -> ForwardOutcome:
    """One response the translator would not build a payload out of at all."""
    return ForwardOutcome(
        response_id="refused-1",
        kind=ForwardOutcomeKind.REFUSED,
        refusals=(
...
def _drain(root: Path, outcomes: tuple[ForwardOutcome, ...]) -> ForwardReport:
    """One drain of exactly these outcomes, dry-run like every drain a flagless run makes."""
    return ForwardReport(
        project_root=root,
        dry_run=True,
        coded_answer_mode=CodedAnswerMode.LENIENT,
...
@pytest.mark.parametrize(
    ("refused", "rejected", "expected_exit"),
    [(False, False, 0), (True, False, 1), (False, True, 1), (True, True, 1)],
)
def test_the_exit_code_is_zero_only_where_nothing_was_refused_or_rejected(
    forward_project: Path, refused: bool, rejected: bool, expected_exit: int
) -> None:
    """A drain that posted nothing and reached the instance with nothing is not a success.

    The four combinations, because the shell script reading this exit code cannot tell a refusal
    from a rejection and should not have to: either the queue drained clean or it did not.
    """
    outcomes: list[ForwardOutcome] = [_accepted_outcome()]
    if refused:
        outcomes.append(_refused_outcome())
    if rejected:
        outcomes.append(_rejected_outcome())

    result, _ = _invoke(["--no-progress"], _drain(forward_project, tuple(outcomes)))

    assert result.exit_code == expected_exit, result.output


def test_a_drain_that_refused_everything_says_it_is_exiting_one(forward_project: Path) -> None:
    """`0 translated, 1 refused, 0 posted` reported success to a shell script until it said otherwise."""
    result, _ = _invoke(["--no-progress"], _drain(forward_project, (_refused_outcome(),)))

    assert result.exit_code == 1, result.output
    assert "refused by the translator; exiting 1" in " ".join(result.output.split())


def test_a_drain_that_refused_everything_exits_one_under_json_too(forward_project: Path) -> None:
    """`--json` renders no narration, so the exit code is the only thing a caller reads the verdict off."""
    mock = AsyncMock(return_value=_drain(forward_project, (_refused_outcome(),)))
    with patch("dhis2w_fhir.service.forward_responses", new=mock):
        result = _runner.invoke(build_app(), ["--json", "fhir", "forward", "--no-progress"])

    assert result.exit_code == 1, result.output
    assert json.loads(result.stdout)["spooled"] == 1
