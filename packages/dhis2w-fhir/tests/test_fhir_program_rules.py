...
def test_a_refusal_naming_a_published_rule_reads_as_the_rules_name() -> None:
    ...
    # One response, so the sentence around the rule stays as DHIS2 wrote it; the rule is still named.
    assert reasons[0].reason == (
        "Generated by ProgramRule (`Show error for high hemoglobin value`) - `The hemoglobin value cannot be above 99`"
    )


def test_a_refusal_naming_a_rule_the_guide_does_not_publish_still_generalises() -> None:
    """A UID this guide holds no rule for is generalised away, so two refusals stay one row."""
    report = ForwardReport(
        project_root=Path("."),
        dry_run=False,
        coded_answer_mode=CodedAnswerMode.LENIENT,
        outcomes=tuple(
            ForwardOutcome(
                ...
                import_outcome=ForwardImportOutcome(
                    status="ERROR",
                    issues=(ForwardImportIssue(error_code="E1300", message=f"Generated by ProgramRule (`{rule}`)"),),
                ),
            )
            for response_id, rule in (("one", "abcdefghij1"), ("two", "abcdefghij2"))
        ),
    )
...
def test_a_refusal_of_one_response_naming_an_unpublished_rule_keeps_the_uid() -> None:
    """There is no second object for the UID to be wrong about, so the reader gets the one DHIS2 named."""
    report = ForwardReport(
        project_root=Path("."),
        dry_run=False,
        coded_answer_mode=CodedAnswerMode.LENIENT,
    ...
    assert report.rejection_reasons[0].reason == "Generated by ProgramRule (`abcdefghij1`)"
