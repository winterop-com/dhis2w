...
async def test_a_warning_rides_back_on_the_created_outcome(capture_client: httpx2.AsyncClient) -> None:
...
async def test_a_stored_warning_is_kept_on_the_receipt(
    capture_client: httpx2.AsyncClient, capture_project: FhirProject
) -> None:
    submission = _temporal_response("DeSymptoms01", {"valueCoding": {"system": SYMPTOM_CODE_SYSTEM, "code": "FEVER"}})
...
async def test_a_strict_facade_refuses_the_code_a_lenient_one_warns_about(
    strict_codes: bool, capture_client: httpx2.AsyncClient
) -> None:
    submission = _temporal_response("DeSymptoms01", {"valueCoding": {"system": SYMPTOM_CODE_SYSTEM, "code": "FEVER"}})
...
async def test_every_json_spelling_of_the_content_type_is_accepted(
    content_type: str, capture_client: httpx2.AsyncClient, aggregate_response: dict[str, Any]
) -> None:
...
async def test_a_receipt_survives_a_restart_over_the_same_project(
    capture_client: httpx2.AsyncClient, capture_project: FhirProject, tracker_response: dict[str, Any]
) -> None:
    created = await capture_client.post("/QuestionnaireResponse", json=tracker_response)
    response_id = created.headers["Location"].rsplit("/", 1)[-1]
...
async def test_the_read_routes_still_answer_for_every_other_type(capture_client: httpx2.AsyncClient) -> None:
...
async def test_creating_any_other_resource_type_is_not_allowed(capture_client: httpx2.AsyncClient) -> None:
