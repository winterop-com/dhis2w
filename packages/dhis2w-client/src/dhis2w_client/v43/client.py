...
from dhis2w_client.v43.complete_data_set_registrations import CompleteDataSetRegistrationsAccessor
...
class Dhis2Client:
    """Async DHIS2 client homed on the v43 tree; the server's major is discovered via /api/system/info on connect.
    ...
    from the `dhis2w_client.v43.*` hand-written tree, so the static type
    chain is v43-pure end-to-end. `connect()` reads `/api/system/info` and,
    ...
    server while the static types stay v43.

    Choose this class when you want v43-typed accessor returns; import
    `dhis2w_client.v41.Dhis2Client` or `dhis2w_client.v42.Dhis2Client` for
    v41- or v42-typed ones. The top-level
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            rebind_accessors_for_version(self, self._version_key, home="v43")
